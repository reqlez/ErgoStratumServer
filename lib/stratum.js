...
const q = new BigNumber('115792089237316195423570985008687907852837564279074904382605163141518161494337');
...
var StratumClient = function(options){
    ...
    this.sendDifficulty = function(difficulty){
        ...
        const clientDifficulty = options.multiplyDifficulty ? 1 : _this.difficulty;
        sendJson({
            id    : null,
            method: "mining.set_difficulty",
            params: [clientDifficulty]//[512],
        });
        return true;
    };
    ...
    this.sendMiningJob = function(jobParams){
        ...
        if(typeof jobParams[6] !== 'string') {
            if (options.multiplyDifficulty) {
                const b = jobParams[6];
                const networkDifficulty = q.dividedBy(b);
                const difficulty = networkDifficulty.multipliedBy(this.difficulty);
                jobParams[6] = q.dividedBy(difficulty).toFixed(0);
            } else {
                jobParams[6] = jobParams[6].toFixed(0);
            }
        }
        sendJson({
            id    : null,
...
var StratumServer = exports.Server = function StratumServer(options, authorizeFn){
    ...
    this.handleNewClient = function (socket){
        ...
        let multiplyDifficulty = false;
        if(socket.localPort && options.ports[socket.localPort]){
            const portOption = options.ports[socket.localPort];
            multiplyDifficulty = portOption.multiplyDifficulty ? portOption.multiplyDifficulty : multiplyDifficulty;
        }
        ...
        var client = new StratumClient(
            {
                ...
                multiplyDifficulty: multiplyDifficulty
            }
        );
